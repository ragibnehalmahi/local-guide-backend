...
const updateMyProfile = catchAsync(async (req: Request, res: Response) => {
  ...
  if (payload.languages && typeof payload.languages === "string") {
    payload.languages = payload.languages.split(",").map((s: string) => s.trim());
  }
  if (payload.expertise && typeof payload.expertise === "string") {
    payload.expertise = payload.expertise.split(",").map((s: string) => s.trim());
  }
  if (payload.travelPreferences && typeof payload.travelPreferences === "string") {
    payload.travelPreferences = payload.travelPreferences.split(",").map((s: string) => s.trim());
  }
...
const getWishlist = catchAsync(async (req: Request, res: Response) => {
  const userId = (req as any).user._id;
  const wishlistItems = await UserServices.getWishlistWithDetails(userId);

...
const updateUser = async (userId, payload, decodedToken) => {
    ...
    // ============ UPDATE USER ============
    const newUpdatedUser = await user_model_1.User.findByIdAndUpdate(userId, { $set: payload }, { new: true, runValidators: true }).select("-password"); // পাসওয়ার্ড বাদ দিয়ে রিটার্ন
    return newUpdatedUser;
};
/**
 * Get my profile

...
class TouristService {
    static async getTouristDashboardStats(touristId) {
        ...
        const tourist = await user_model_1.User.findById(touristId).select("wishlist").lean();
        if (!tourist)
        ...
        const statsResult = await booking_model_1.Booking.aggregate([
            { $match: { tourist: touristId } },
            {
                $group: {
                    ...
                    totalBookings: { $sum: 1 },
                    upcomingTours: {
                        $sum: { $cond: [{ $and: [{ $eq: ["$status", booking_model_1.BookingStatus.CONFIRMED] }, { $gte: ["$date", new Date()] }] }, 1, 0] }
                    },
                    completedTours: {
                        $sum: { $cond: [{ $eq: ["$status", booking_model_1.BookingStatus.COMPLETED] }, 1, 0] },
                    },
                    totalSpent: {
                        $sum: { $cond: [{ $eq: ["$paymentStatus", booking_model_1.PaymentStatus.PAID] }, "$totalPrice", 0] },
                    }
                }
            }
        ]);
        ...
        const recentBookings = await booking_model_1.Booking.find({ tourist: touristId })
